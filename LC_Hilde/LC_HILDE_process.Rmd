...
Execute `filter_tifs_by_pg_intervals`function:
...
#### Batch Processing Land Cover Rasters Across Multiple Years

The `batch_process_landcover_in_tiles_proportional()` function automates the extraction of proportional land cover areas for all PRIO-GRID cells by looping over a list of raster files—each representing a different year of land cover data.

This function applies the previously defined `process_landcover_in_tiles_proportional()` to each file in the input list and compiles the results into a single data frame.
...
Execute Batch Land Cover Extraction

We run the `batch_process_landcover_in_tiles_proportional()` function to extract proportional land cover data across all raster files. This compiles results from multiple years, informed by the `filtered_files`variable.

```{r}
year_compile_df <- batch_process_landcover_in_tiles_proportional(filtered_files, pg_vect, tile_df, runtest=TRUE)
```


```{r}
# Print unique years available in the data
unique_years <- sort(unique(year_compile_df$year))
print(unique_years)
```

#### Create Raster Stacks of Land Cover Proportions by Year

We now convert the multi-year data frame of proportional land cover areas (`year_compile_df`) into a list of raster stacks using the `batch_create_landcover_area_stack()` function. Each stack corresponds to a unique year and contains:

- One base layer representing `pg_id` values.
- Additional layers for each land cover type (e.g., `lc_10`, `lc_22`), where pixel values indicate the proportion of the PRIO-GRID cell covered by that class.
...
```
We generate a list of yearly raster stacks using `batch_create_landcover_area_stack()`, where each element represents land cover proportions per PRIO-GRID cell for a specific year.
...
```{r}
names(landcover_stack_list[["2002"]])

```

```{r}
plot(landcover_stack_list[["2002"]][["lc_33"]])
```
...
#unzipped_subdir <- list.files(unzipped_directory, full.names = TRUE) //Used to identify "hildap_GLOB-v1.0_lulc-states"

states_dir <- file.path(unzipped_directory, "hildap_vGLOB-1.0_geotiff_wgs84", "hildap_GLOB-v1.0_lulc-states")

# List all files in that directory (non-recursive since you’re targeting a specific folder)
#state_files <- list.files(states_dir, full.names = TRUE) //Used to identify tif files in directory
