...
and verifies that citation keys exist in `inst/REFERENCES.bib`.
...
- `name` must match the string passed to `names(r)` in `gen_*()` and
  equal `gen_{name}` without the prefix.
- `static` is `TRUE` for variables without a time dimension, `FALSE` for
  time-varying.
- `source_ids` is the UUID from `sources.csv`. Multiple sources are
  comma-separated.
...
Add tests to `tests/testthat/`. For a gen\_\* function, the minimal
test:
...
Add row to `data_raw/sources.csv` with UUID, license, URLs, citation
keys
