# Authors and Citation

## Authors

- **Jonas Vestby**. Author, maintainer.
  [](https://orcid.org/0000-0002-3054-977X)

- **Andreas Forø Tollefsen**. Author.
  [](https://orcid.org/0000-0002-0864-5132)

- **Karim Bahgat**. Contributor.  
  Co-author of PG2

- **Peace Research Institute Oslo (PRIO)**. Copyright holder.

- **Håvard Strand**. Contributor.

- **Scott Gates**. Contributor.

- **Halvard Buhaug**. Contributor.

- **Kristine Helskog**. Contributor.

- **Garret Benz**. Contributor.

- **Ingvild Kjelstrup Smestad**. Contributor.

- **Tiril Rahn**. Contributor.

- **Christian Oswald**. Contributor.

- **Peder Landsverk**. Contributor.

- **Vilde Bergstad Larsen**. Contributor.

## Citation

Source:
[`inst/CITATION`](https://github.com/prio-data/priogrid/blob/3.0.2/inst/CITATION)

Tollefsen AF, Strand H, Buhaug H (2012). “PRIO-GRID: A Unified Spatial
Data Structure.” *Journal of Peace Research*, **49**(2), 363–374.
[doi:10.1177/0022343311431287](https://doi.org/10.1177/0022343311431287).

    @Article{,
      title = {PRIO-GRID: A Unified Spatial Data Structure},
      author = {Andreas Forø Tollefsen and Håvard Strand and Halvard Buhaug},
      year = {2012},
      journal = {Journal of Peace Research},
      volume = {49},
      number = {2},
      pages = {363--374},
      doi = {10.1177/0022343311431287},
    }

Vestby J, Tollefsen AF, Helskog K, Benz G (2025). *priogrid: the
R-package*. R package version 3.0.2,
<http://prio-data.github.io/priogrid>.

    @Manual{,
      title = {priogrid: the R-package},
      author = {Jonas Vestby and Andreas Forø Tollefsen and Kristine Helskog and Garret Benz},
      year = {2025},
      note = {R package version 3.0.2},
      url = {http://prio-data.github.io/priogrid},
    }
