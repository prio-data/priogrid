...
```
## Here we consider pg options (date) parameters:
```{r}
# Get the layer names from the raster stack
layer_names <- names(cru_stack)

# Extract YYYY-MM from "tmp_YYYY_MM" and convert to proper Date
layer_dates <- ymd(paste0(sub("tmp_", "", layer_names), "-01"))

pg_intervals <- pg_date_intervals()


# Determine which layer_dates fall within any pg_interval
keep_layers <- sapply(layer_dates, function(d) any(d %within% pg_intervals))

cru_stack_filtered <- cru_stack[[which(keep_layers)]]

layer_names_filtered <- names(cru_stack_filtered)
```
...
```{r}
mask_cru_to_pg_stack <- function(cru_stack_filtered, pg_grid, ym_df = NULL) {
  library(terra)
