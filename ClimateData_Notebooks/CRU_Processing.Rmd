...
# Ensure both layers are using WGS84 and geodesic calculations are enabled
sf_use_s2(TRUE)  # enable spherical geometry engine

# Compute distance matrix in meters (geodesic)
dist_matrix_m <- st_distance(pg_centroids, raster_pts_sf)  # returns units in meters

# For each PG centroid, get the minimum distance to any raster point
min_distances_m <- apply(dist_matrix_m, 1, min)

# Summarize distances
cat("\n=== Minimum Distance (in meters) from PG Centroid to Raster Cell Center ===\n")
print(summary(min_distances_m))

# Threshold: within 5 meters is considered perfect alignment
tolerance_m <- 5  # meters
num_within <- sum(min_distances_m < tolerance_m)
total <- length(min_distances_m)

cat(sprintf("\nPG centroids within %.1f meters of raster cell center: %d of %d\n", tolerance_m, num_within, total))

if (num_within == total) {
  cat("✅ All PG centroids are aligned within tolerance.\n")
} else {
  cat("⚠️ Some PG centroids are offset more than", tolerance_m, "meters.\n")
}
...
mismatch_idx <- which(min_distances_m >= tolerance_m)
...
raster_pts <- as.points(dec_2020_mask)  # already masked to PG extent
