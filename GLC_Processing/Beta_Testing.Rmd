...
glc_landcover <- function(landcovertype, beta_test = FALSE) {
  # Store and temporarily adjust memory allocation
  memfrac_option <- terra::terraOptions(verbose = FALSE)$memfrac
  terra::terraOptions(memfrac = 0.8)
  
  # Step 1: Read and prepare rasters
  tif_files <- read_glc_v2(beta_test = beta_test)
  ras_list_filtered <- prepare_glc_layers(tif_files)
  
  # Step 2: Create a global template grid (lazy, not materialized)
  template <- terra::rast(
    extent = c(-180, 180, -90, 90),
    resolution = terra::res(ras_list_filtered[[1]]),
    crs = "EPSG:4326"
  )
  
  # Step 3: Align each tile to global extent (adds NA placeholders lazily)
  ras_aligned <- lapply(ras_list_filtered, function(r) {
    terra::extend(r, template)
  })
  
  # Step 4: Combine aligned tiles into a single lazy SpatRaster
  r <- terra::rast(ras_aligned)
  
  # Step 5: Notify about processing
  message("Running landcover computation on ", length(ras_aligned), " aligned tiles.")
  message("Missing or empty regions handled as NA during aggregation.")
  
  # Step 6: Run the transformation
  res <- robust_transformation(r, function(x) mean(x == landcovertype))
  
  # Step 7: Restore memory settings and return result
