...
inspect_glc_tiles <- function(beta_test = TRUE, sample_n = NULL) {
  ...
  stats_list <- lapply(seq_along(ras_list_filtered), function(i) {
    ...
    values_sample <- tryCatch({
      ...
      message("    ⚠️ Could not read values for ", fname, ": ", e$message)
      return(NULL)
    })
...
glc_landcover <- function(landcovertype, beta_test = FALSE) {
  ...
  message("Each tile will be processed individually to prevent memory overload.\n")

  # 2️⃣ Ensure CRS consistency
  ras_list_filtered <- lapply(ras_list_filtered, function(r) {
  ...
  # 3️⃣ Process tiles independently (with progress messages)
  ...
  for (i in seq_along(ras_list_filtered)) {
    ...
    # Run robust transformation on one tile
    tile_results[[i]] <- tryCatch(
      robust_transformation(tile, function(x) mean(x == landcovertype)),
      error = function(e) {
  ...
  # 4️⃣ Drop empty results
  tile_results <- Filter(Negate(is.null), tile_results)

  # 5️⃣ Merge processed (aggregated) outputs — not raw rasters
  if (length(tile_results) > 1) {
    message("\n🧩 Merging ", length(tile_results), " aggregated tiles into one output layer...")
    res <- do.call(terra::mosaic, c(tile_results, fun = "mean"))
  } else {
    res <- tile_results[[1]]
  }
...
cat("\n🧭 Inspecting raster:", basename(tif_files[1]), "\n")
cat("--------------------------------------------------\n")

# 1️⃣ Check if the raster is categorical
is_cat <- terra::is.factor(r)
cat("Is this a categorical (factor) raster?  ", is_cat, "\n")

# 2️⃣ Check number of layers
num_layers <- terra::nlyr(r)
cat("Number of layers: ", num_layers, "\n")

# 3️⃣ See CRS and extent (just for orientation)
cat("CRS: ", terra::crs(r), "\n")
cat("Extent: ", toString(terra::ext(r)), "\n")

# 4️⃣ Data type (e.g., INT1U, INT2S, FLT4S, etc.)
cat("Data type: ", terra::datatype(r), "\n")

# 5️⃣ Peek at unique values (first layer only)
