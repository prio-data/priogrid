...
# Filter each raster in ras_list
ras_list_filtered <- lapply(ras_list, function(r) {
  layer_years <- as.numeric(names(r))     # extract layer years
  keep_layers <- layer_years %in% years_to_keep
  
  if (any(keep_layers)) {
    r <- r[[keep_layers]]                 # subset to matching years
  } else {
...
for (i in seq_along(ras_list_filtered)) {
  yrs <- as.numeric(names(ras_list_filtered[[i]]))
  names(ras_list_filtered[[i]]) <- as.Date(
    paste(yrs, month_to_use, day_to_use, sep = "-")
  )
}
...
names(ras_list_filtered[[1]])[1:10]
