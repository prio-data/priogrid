#' sgdi
#'
#' Gives the subnational gender development index (sgdi) within each grid cell, based on sgdi data v. 4.
...
#'
#' Links to original data: https://globaldatalab.org/sgdi/shapefiles/ and https://globaldatalab.org/sgdi/download_files/.
#'
...
#' @param variable one of c("sgdi", msch", "esch", "lifexp", "lgnic").
...
#'
#' @export
## initially set to TRUE, set FALSE to check if all else is running smoothly
gen_sgdi <- function(input_folder, variable = "sgdi", add_missing_geometries = FALSE){
  
  shape_zip <- file.path(input_folder, "input", "gdl", "GDL Shapefiles V6.2.zip")
  ...
  sgdi <- read.csv(file.path(input_folder, "input", "gdl", "SHDI-SGDI-Total 7.0.csv"))
  sgdi <- sgdi %>% dplyr::rename_all(tolower)
  
  # Countries without subnational data
  no_subnat <- sgdi %>% dplyr::group_by(country) %>% dplyr::summarize(n_unique = length(unique(gdlcode))) %>% dplyr::filter(n_unique == 1)
  ...
  sgdi <- dplyr::bind_rows(sgdi_national, sgdi)
  sgdi <- dplyr::left_join(sgdi, geom, by = "gdlcode") %>% sf::st_as_sf()
  
  if(add_missing_geometries){
    sgdi$empty <- sf::st_is_empty(sgdi)
    sgdi_empty <- sgdi %>% dplyr::filter(empty)
    sf::st_geometry(sgdi_empty) <- NULL
    
    path_miss <- file.path(input_folder, "input", "sgdi", "data", "missing_geom")
    ...
    sgdi_missing <- dplyr::inner_join(miss, sgdi_empty, by = "gdlcode")
    
    sgdi <- sgdi %>% dplyr::filter(!empty)
    
    sgdi <- rbind(sgdi, sgdi_missing)
    
    sgdi <- sgdi %>% dplyr::arrange(gdlcode, year) %>% dplyr::select(-empty)
  }
  
  sgdi_sum <- priogrid::panel_to_pg(sgdi, timevar = "year", variable = variable, need_aggregation = TRUE, missval = -1, fun = "sum")
  sgdi_count <- priogrid::panel_to_pg(sgdi, timevar = "year", variable = variable, need_aggregation = TRUE, missval = -1, fun = "count")
  sgdi_count <- sgdi_count %>% dplyr::rename("count" = !!variable)
  sgdi_sum <- dplyr::left_join(sgdi_sum, sgdi_count, by = c("x", "y", "pgid", "year"))
  
  sgdi_sum[[variable]] <- sgdi_sum[[variable]] / sgdi_sum$count
  sgdi_sum$count <- NULL
  sgdi_sum$year <- as.numeric(sgdi_sum$year)
  return(sgdi_sum)
}
...
#' msch
#'
#' Gives the mean years of schooling for population aged 25+ within each grid cell, based on sgdi data v. 4. Available from 1990 to 2018.
#'
#' Links to original data: https://globaldatalab.org/sgdi/shapefiles/ and https://globaldatalab.org/sgdi/download_files/.
#'
#' Please cite: Smits, J., Permanyer, I. The Subnational Human Development Database. Sci Data 6, 190038 (2019). https://doi.org/10.1038/sdata.2019.38
...
#'
#' @export
gen_msch <- function(input_folder, add_missing_geometries = TRUE){ gen_sgdi(input_folder, variable = "msch", add_missing_geometries = add_missing_geometries) }


#' esch
#'
#' Gives the expected years of schooling for children aged 6 within each grid cell, based on sgdi data v. 4. Available from 1990 to 2018.
#'
#' Links to original data: https://globaldatalab.org/sgdi/shapefiles/ and https://globaldatalab.org/sgdi/download_files/.
#'
#' Please cite: Smits, J., Permanyer, I. The Subnational Human Development Database. Sci Data 6, 190038 (2019). https://doi.org/10.1038/sdata.2019.38
...
#'
#' @export
gen_esch <- function(input_folder, add_missing_geometries = TRUE){ gen_sgdi(input_folder, variable = "esch", add_missing_geometries = add_missing_geometries) }


#' lifexp
#'
#' Gives the life expectancy at birth within each grid cell, based on sgdi data v. 4. Available from 1990 to 2018.
#'
#' Links to original data: https://globaldatalab.org/sgdi/shapefiles/ and https://globaldatalab.org/sgdi/download_files/.
#'
#' Please cite: Smits, J., Permanyer, I. The Subnational Human Development Database. Sci Data 6, 190038 (2019). https://doi.org/10.1038/sdata.2019.38
...
#'
#' @export
gen_lifexp <- function(input_folder, add_missing_geometries = TRUE){ gen_sgdi(input_folder, variable = "lifexp", add_missing_geometries = add_missing_geometries) }


#' gnic
#'
#' Gives the Gross National Income per capita in thousands of USD (2011 PPP) within each grid cell, based on sgdi data v. 4. Available from 1990 to 2018.
#'
#' Links to original data: https://globaldatalab.org/sgdi/shapefiles/ and https://globaldatalab.org/sgdi/download_files/.
#'
#' Please cite: Smits, J., Permanyer, I. The Subnational Human Development Database. Sci Data 6, 190038 (2019). https://doi.org/10.1038/sdata.2019.38
...
#'
#' @export
gen_gnic <- function(input_folder, add_missing_geometries = TRUE){ gen_sgdi(input_folder, variable = "gnic", add_missing_geometries = add_missing_geometries) }
