...
DROP TABLE IF EXISTS spei3_gdm_georef;
CREATE TEMP TABLE spei3_gdm_georef AS
(
...
-- ADD INDEX
CREATE INDEX spei3_gdm_georef_idx ON spei3_gdm_georef USING gist(ST_ConvexHull(rast));
ANALYZE spei3_gdm_georef;
...
CREATE OR REPLACE FUNCTION calcYearlySPEI3(integer, integer) returns integer as 
$$
DECLARE
	_from ALIAS FOR $1;
	_to ALIAS FOR $2;
	result integer;
BEGIN
	DROP TABLE IF EXISTS dev.spei3_gdm;
	CREATE TABLE dev.spei3_gdm(	gid integer, "year" integer,
					jan decimal,
					feb decimal,
					mar decimal,
					apr decimal,
	...
	FOR i IN _from.._to LOOP
		...
		FROM priogrid_land AS p, spei3_gdm_georef AS pr
		WHERE p.centroid && pr.rast -- bboxes intersect (faster)
		;
	END LOOP;

	-- set null values
	UPDATE dev.spei3_gdm SET jan = NULL WHERE jan < -1111.0 OR jan > 1111.0;
	UPDATE dev.spei3_gdm SET feb = NULL WHERE feb < -1111.0 OR feb > 1111.0;
	UPDATE dev.spei3_gdm SET mar = NULL WHERE mar < -1111.0 OR mar > 1111.0;
	UPDATE dev.spei3_gdm SET apr = NULL WHERE apr < -1111.0 OR apr > 1111.0;
	UPDATE dev.spei3_gdm SET may = NULL WHERE may < -1111.0 OR may > 1111.0;
	UPDATE dev.spei3_gdm SET jun = NULL WHERE jun < -1111.0 OR jun > 1111.0;
	UPDATE dev.spei3_gdm SET jul = NULL WHERE jul < -1111.0 OR jul > 1111.0;
	UPDATE dev.spei3_gdm SET aug = NULL WHERE aug < -1111.0 OR aug > 1111.0;
	UPDATE dev.spei3_gdm SET sep = NULL WHERE sep < -1111.0 OR sep > 1111.0;
	UPDATE dev.spei3_gdm SET oct = NULL WHERE oct < -1111.0 OR oct > 1111.0;
	UPDATE dev.spei3_gdm SET nov = NULL WHERE nov < -1111.0 OR nov > 1111.0;
	UPDATE dev.spei3_gdm SET des = NULL WHERE des < -1111.0 OR des > 1111.0;
...
-- RUN FUNCTION
SELECT calcYearlySPEI3(1950, 2014);
