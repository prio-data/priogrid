...
DROP TABLE IF EXISTS spei3_base_georef;
CREATE TEMP TABLE spei3_base_georef AS
(
...
SELECT rid, ST_SetGeoReference(rast, 
		...
		0, 0) -- skewx, skewy
		AS rast
FROM orig.spei3_base
);


-- ADD INDEX
CREATE INDEX spei3_base_georef_idx ON spei3_base_georef USING gist(ST_ConvexHull(rast));
ANALYZE spei3_base_georef;
...
DECLARE
	...
	result integer;
BEGIN
	DROP TABLE IF EXISTS dev.spei3_base;
	CREATE TABLE dev.spei3_base(	gid integer, "year" integer,
					jan decimal,
					feb decimal,
					mar decimal,
					apr decimal,
	...
	FOR i IN _from.._to LOOP
		...
		FROM priogrid_land AS p, spei3_base_georef AS pr
		WHERE p.centroid && pr.rast -- bboxes intersect (faster)
		;
	END LOOP;

	-- set null values
	UPDATE dev.spei3_base SET jan = NULL WHERE jan > 1111.0;
	UPDATE dev.spei3_base SET feb = NULL WHERE feb > 1111.0;
	UPDATE dev.spei3_base SET mar = NULL WHERE mar > 1111.0;
	UPDATE dev.spei3_base SET apr = NULL WHERE apr > 1111.0;
	UPDATE dev.spei3_base SET may = NULL WHERE may > 1111.0;
	UPDATE dev.spei3_base SET jun = NULL WHERE jun > 1111.0;
	UPDATE dev.spei3_base SET jul = NULL WHERE jul > 1111.0;
	UPDATE dev.spei3_base SET aug = NULL WHERE aug > 1111.0;
	UPDATE dev.spei3_base SET sep = NULL WHERE sep > 1111.0;
	UPDATE dev.spei3_base SET oct = NULL WHERE oct > 1111.0;
	UPDATE dev.spei3_base SET nov = NULL WHERE nov > 1111.0;
	UPDATE dev.spei3_base SET des = NULL WHERE des > 1111.0;
...
-- RUN FUNCTION
SELECT calcYearlySpei3(1946, 2013);
