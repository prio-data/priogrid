...
DROP TABLE IF EXISTS spi3_recentered;
CREATE TEMP TABLE spi3_recentered 
AS
(
...
FROM roll_raster_table('orig.spi3', 'rast', 180)  -- roll the pacific and americas off the right edge and back around the left edge
);
...
-- ADD INDEX
CREATE INDEX spi3_recentered_idx ON spi3_recentered USING gist(ST_ConvexHull(rast));
ANALYZE spi3_recentered;
...
CREATE OR REPLACE FUNCTION calcYearlySPI3(integer, integer) returns integer as 
$$
DECLARE
	_from ALIAS FOR $1;
	_to ALIAS FOR $2;
	result integer;
BEGIN
	DROP TABLE IF EXISTS dev.spi3;
	CREATE TABLE dev.spi3(	gid integer, "year" integer,
					jan decimal,
					feb decimal,
					mar decimal,
					apr decimal,
	...
	FOR i IN _from.._to LOOP
		...
		FROM priogrid_land AS p, spi3_recentered AS pr
	...
	-- set null values
	UPDATE dev.spi3 SET jan = NULL WHERE jan < -999;
	UPDATE dev.spi3 SET feb = NULL WHERE feb < -999;
	UPDATE dev.spi3 SET mar = NULL WHERE mar < -999;
	UPDATE dev.spi3 SET apr = NULL WHERE apr < -999;
	UPDATE dev.spi3 SET may = NULL WHERE may < -999;
	UPDATE dev.spi3 SET jun = NULL WHERE jun < -999;
	UPDATE dev.spi3 SET jul = NULL WHERE jul < -999;
	UPDATE dev.spi3 SET aug = NULL WHERE aug < -999;
	UPDATE dev.spi3 SET sep = NULL WHERE sep < -999;
	UPDATE dev.spi3 SET oct = NULL WHERE oct < -999;
	UPDATE dev.spi3 SET nov = NULL WHERE nov < -999;
	UPDATE dev.spi3 SET des = NULL WHERE des < -999;
...
-- RUN FUNCTION
SELECT calcYearlySPI3(1979, 2014);
