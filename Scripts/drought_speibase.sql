...
CREATE TEMP TABLE spei1months AS
	(
	WITH source AS (SELECT * FROM dev.spei1_base)
...
UPDATE spei1months SET monthnum = (year - 1946) * 12 + month;
...
CREATE TEMP TABLE spei3months AS
	(
	WITH source AS (SELECT * FROM dev.spei3_base)
...
UPDATE spei3months SET monthnum = (year - 1946) * 12 + month;
...
ON s1.gid = r.gid AND s1.monthnum = ((s1.year - 1946) * 12 + r.startmonth)   -- the 1 month indicator is chosen as months since start of the data for that year + the rainseason startmonth for that gid
...
DROP TABLE IF EXISTS drought_speibase;
CREATE TABLE drought_speibase
AS
(
