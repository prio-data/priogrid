---
title: "Custom Spatial and Temporal Configurations"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Custom Spatial and Temporal Configurations}
...
PRIOGRID's default release uses 0.5-degree global cells with annual time steps. This tutorial shows how to change the spatial extent, resolution, projection, and temporal granularity — useful for testing the modifiable areal unit problem (MAUP), creating tailored regional datasets, or generating monthly or quarterly data.

## The Configuration Object

All spatial and temporal parameters are held in a `pg_config` object. The current session config is accessed with:

```{r current-config}
pg_current_config()
```

Create a custom config with `pg_config()`:
...
> **Date convention:** Use the last day of each temporal increment. For yearly data with December measurement dates, use `1990-12-31`. For monthly data with January measurement dates, use `1990-01-31`.

## Updating the Session Config

`pg_set_config()` modifies the current session config in place:
...
- **Temporal hash** — derived from `temporal_resolution`, `start_date`, and `end_date`

These hashes uniquely identify a configuration without encoding the full parameter set in the folder name. The file path looks like:
...
```{r hashes, echo=TRUE, results='markup'}
...
If you change any spatial parameter, the spatial hash changes and PRIOGRID will treat it as a new, independent build. This prevents accidental mixing of data from incompatible configurations.

> **Important:** PRIOGRID cell IDs (`pgid`) depend on the spatial configuration. Data from different configs cannot be joined by `pgid`.

## Where Custom Data Is Stored

`pgout_path()` returns the storage path for the current config:

```{r pgout-path, eval=FALSE}
...
`calc_pg()` calculates variables and saves them to disk. It calls the `gen_*()` function for each variable:

```{r calc-pg, eval=FALSE}
cfg <- pg_config(
  nrow = 180,
  ncol = 360,
...
By default, existing files are skipped. Use `overwrite = TRUE` to recalculate:

```{r calc-overwrite, eval=FALSE}
calc_pg("ruggedterrain_elevation_mean", overwrite = TRUE, config = cfg)
```

After calculation, load results with `load_pgvariable()`:

```{r load-custom, eval=FALSE}
...
`pg_list_custom()` shows all custom builds currently stored on disk:
...
Load custom builds into tabular form the same way as the official release:

```{r read-custom, eval=FALSE}
cfg <- pg_config(nrow = 180, ncol = 360)
...
All `read_pg_timevarying()` subsetting arguments (`years`, `start_date`, `end_date`, `pgids`,
`extent`, `variables`) work identically with custom configs. `extent` is always expressed in
lon/lat (EPSG:4326), matching the `config$extent` convention — reprojection is handled
automatically for non-4326 configs:

```{r read-custom-subset, eval=FALSE}
cfg <- pg_config(nrow = 180, ncol = 360)
...
| `load_pgvariable()` | Auto-downloads | Requires `calc_pg()` first |

## Next Steps

- [Understanding PRIOGRID Metadata](metadata.html) — sources, variables, and search
- [Contributing to PRIOGRID](contributing.html) — writing `gen_*()` functions and registering new sources
