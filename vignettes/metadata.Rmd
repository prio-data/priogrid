...
each built COG as `pg_*` GDAL metatags by `save_pgvariable()`. Built COGs additionally carry
computed `pg_colormap`/`pg_value_min`/`pg_value_max`/`pg_value_mean`/`pg_value_std`/`pg_nunique`/`pg_class_values`
(not authored columns). These map onto rio-tiler `rescale`=`pg_value_min`,`pg_value_max` and
