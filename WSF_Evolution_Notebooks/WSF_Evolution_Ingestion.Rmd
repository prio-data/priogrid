...
process_wsf_tile <- function(wsf_file, priogrid_raster) {
  message("Processing: ", basename(wsf_file))
  
  # Load the WSF Evolution raster tile
  wsf <- rast(wsf_file)
  ...
  # Convert PRIO-GRID raster to polygons
  priogrid_vect <- as.polygons(priogrid_raster, na.rm = TRUE)
  names(priogrid_vect) <- "pg_id"

  # Crop to WSF extent to reduce processing
  priogrid_vect <- crop(priogrid_vect, ext(wsf))

  # Estimate pixel area in km² (approx. 30m resolution → ~0.0009 km²)
  px_area_km2 <- res(wsf)[1] * res(wsf)[2] / 1e6  # m² to km²

  # Extract WSF pixel values and associated pg_id
  df_raw <- terra::extract(wsf, priogrid_vect, cells = TRUE, na.rm = TRUE)
  colnames(df_raw) <- c("pg_id", "year", "cell")

  # Drop background pixels (value <= 0)
  df_raw <- df_raw %>% filter(year > 0)

  # Aggregate: count pixels by pg_id and year
  df_sum <- df_raw %>%
    group_by(pg_id, year) %>%
    summarise(pixel_count = n(), .groups = "drop") %>%
    mutate(built_area_km2 = pixel_count * px_area_km2)

  # Pivot wider: one row per pg_id, one column per year
  df_wide <- df_sum %>%
    select(pg_id, year, built_area_km2) %>%
    pivot_wider(names_from = year, values_from = built_area_km2, values_fill = 0) %>%
    arrange(pg_id)
...
```{r}
wsf_summary_df <- process_wsf_tile("/Users/gbenz/Documents/priogrid/World Settlement Footprint Evolution/updated 2025/f52c5d59-8867-4b5b-8504-bcc6dcf6839d/WSFevolution_v1_32_16.tif", pg_grid)
...
# Load a WSF Evolution tile
wsf_file <- "/Users/gbenz/Documents/priogrid/World Settlement Footprint Evolution/updated 2025/f52c5d59-8867-4b5b-8504-bcc6dcf6839d/WSFevolution_v1_32_16.tif"  # Update to your actual path
wsf <- rast(wsf_file)
...
# --- 5. Extract raster values per PRIO-GRID polygon ---
df_raw <- terra::extract(wsf, pg_vect, bind = TRUE, cells = TRUE, na.rm = TRUE)
colnames(df_raw) <- c("pg_id", "year", "cell")

# --- 5a. Fix: Ensure year values are treated as integers ---
df_raw$year <- as.integer(round(df_raw$year))  # <-- This is critical

# --- 6. Summarize by year and pg_id ---
px_area_km2 <- res(wsf)[1] * res(wsf)[2] * (111.32^2)

summary_df <- df_raw %>%
  filter(year > 1900 & year < 2100) %>%  # Optional sanity filter
  group_by(pg_id, year) %>%
  summarise(pixel_count = n(), .groups = "drop") %>%
  mutate(built_area_km2 = pixel_count * px_area_km2) %>%
  arrange(pg_id, year)

# --- 7. View summary table ---
print(summary_df)
