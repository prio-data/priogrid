...
-   [Getting Started]{.underline} show you how to set up the package, download the official release, and read PRIO-GRID as tabular data.

-   [Accessing PRIO-GRID as Rasters]{.underline} show you how to work with individual variables as spatial rasters using `r terra` for spatial analysis and mapping.

-   [Custom Spatial and Temporal Configuraions]{.underline} show you how to change the resolution, extent, projection, or time step to build tailored regional datasets or conduct sensitivity analyses.

-   [Understanding PRIO-GRID Metadata]{.underline} show you the sources and variables behind the grid with `r pgsources`, `r pgvariables`, and `r pgsearch()`.

-   [Citation and Bibliography]{.underline} show you how to automatically retrieve every citation the data you used requires, and how to export it as BibLaTeX for your paper.
...
-   **Cite the data you use.** PRIO-GRID stands on the shoulders of many upstream data providers that must be cited and credited. The package makes this easy with `r pgcitations()` (see the [Citations tutorial]{.underline}).

-   **Send us issues.** If you encounter errors, failed downloads, or bugs, [open an issue]{.underline} (see [Reporting a bug]{.underline}.

-   **Suggest a data source or variable.** If you know a great open dataset that should be incorporated into PRIO-GRID, please suggest it for us (see [Suggesting a data source]{.underline}).
...
-   **Contribute code**. If you want to fix a bug or add a data source yourself we welcome contributions (see [Contributing code]{.underline}).
...
We welcome ideas for new open datasets to incorporate into PRIO-GRID. If you want a full overview of what already exists in PRIO-GRID before you suggesting, have a look at the [Metadata tutorial]{.underline} and `r pgsearch()` to see whether something similar already exists. Then [open an issue]{.underline} with:
...
4.  Open a pull request on `r prio-data/priogrid`, describing what it does and why.

Additional notes for contributors:

-   **The package is set up for development with `r devtools`.** `r devtools::load_all()` loads your working copy, `r devtools::test()` and `r devtools::document()` can also be useful commands to run before opening a pull request.

-   **Match the existing style.** PRIO-GRID follows the tidyverse style guide and uses established naming patterns such as `r read_*()`, `r gen_*()`, `r pg_*()`. Documentation is written with `r roxygen2`
...
If your contribution is a new data source or variable, or [Contributing to PRIOGRID tutorial]{.underline} walks through every step from registering the source, writing `r read_*()` and `r gen_*()` functions, adding tests, and verifying the result.
...
**Questions, bugs, and ideas:** the [Issue Tracker.]{.underline}
...
The `r priogrid` code is released under the MIT License and the PRIO-GRID dataset is released under ODC-BY v1.0. Data derived from third-party sources remains subject to those providers' licenses. See `r LICENSE.md` for citation details. By contributing, you agree your work is offered under these same terms.
